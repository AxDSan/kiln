...
fn a_hex_literal_is_the_bits_that_were_written() {
    ...
    for (expr, want) in [
        ...
        assert_eq!(
            value_of(&dir, &tag, "", "print_int", expr),
            want,
            "for `{expr}`"
        );
    }
...
fn a_hex_literal_widens_with_zeros() {
    ...
    assert_eq!(
        value_of(
            &dir,
            "w3",
            "  var v: int64 = 0xFFFF_FFFF_FFFF_FFFF",
            "print_int64",
            "v"
        ),
        "-1"
...
fn a_malformed_bit_pattern_is_refused() {
    ...
    for (body, needle) in [
        ...
        (
            "  call print_int(0xFFFF_FFFF_FFFF_FFFF_F)",
            "more than 64 bits",
        ),
    ] {
...
fn shr_keeps_the_sign_and_ushr_does_not() {
    ...
    assert_eq!(
        value_of(&dir, "s2", "", "print_int", "-16 ushr 2"),
        "1073741820"
    );
    assert_eq!(
        value_of(&dir, "s3", "  var v: int64 = -16", "print_int64", "v shr 2"),
        "-4"
    );
    assert_eq!(
        value_of(
...
fn a_shift_count_is_checked_when_it_is_written_down() {
    ...
    assert_eq!(
        value_of(&dir, "c2", "  var n: int = 33", "print_int", "1 shl n"),
        "2"
    );
}
...
fn loword_hiword_and_a_flag_test() {
    ...
    assert_eq!(
        value_of(&dir, "lo", decls, "print_int64", "wparam band 0xFFFF"),
        "2"
    );
    assert_eq!(
        value_of(
...
fn a_non_integer_operand_is_named_and_refused() {
    ...
    for (body, needle) in [
        (
            "  call print_double(1.5 band 2.0)",
            "`band` works on int and int64 values; its left side is double",
        ),
        ("  call print_int(1 bor 2.0)", "its right side is double"),
        (
            "  var b: bool = true\n  call print_int(b bxor 1)",
            "its left side is bool",
        ),
        (
            "  call print_text(\"a\" bor \"b\")",
            "its left side is text",
        ),
        (
            "  var p: ptr = ptr_null()\n  call print_int(p band 1)",
            "its left side is ptr",
        ),
        (
            "  call print_double(bnot 1.5)",
            "`bnot` flips the bits of an int or an int64, got double",
        ),
        (
            "  call print_int(1 shl 1.5)",
            "must be int or int64, not double",
        ),
    ] {
...
fn the_operator_words_are_still_ordinary_names() {
    ...
    for body in [
        ...
        assert!(
            said.contains("BNot"),
            "the error should point at `bnot`, got:\n{said}"
        );
    }
    // The operator itself, including in front of a parenthesised expression.
    assert_eq!(
        value_of(&dir, "n5", "", "print_int", "bnot (1 bor 2)"),
        "-4"
    );
