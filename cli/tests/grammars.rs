...
const K2_KEYWORDS: &[&str] = &[
    ...
    "class", "record", "struct", "enum", "interface", "form", "new", "this", "return", "if",
    "else", "switch", "case", "default", "for", "foreach", "in", "while", "do", "break",
    "continue", "defer", "is", "as", "ref", "out",
];

#[test]
fn the_vscode_grammar_knows_the_k2_keywords() {
    let g = std::fs::read_to_string(
        repo_root().join("editors/vscode/syntaxes/kiln.tmLanguage.json"),
    )
    .unwrap();
